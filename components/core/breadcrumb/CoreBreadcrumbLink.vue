...
import { type ProvideBeradcrumb } from "./CoreBreadcrumb.vue"
...
const computedClass = computed(() => [
   props.index > 0 && "relative ml-5 pl-0.5",
   breadcrumb &&
      !breadcrumb.light &&
      props.index > 0 &&
   ...
   props.index > 0 && "dark:before:bg-chevron-white",
   breadcrumb &&
      !breadcrumb.light &&
      props.active &&
      "text-slate-800 cursor-text dark:text-slate-400",
   breadcrumb && breadcrumb.light && props.active && "text-white/70",
])
</script>
